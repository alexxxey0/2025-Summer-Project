...
function EditUser(props) {
    ...
    return (
        <div className="mt-16">
            <Link href={"/user_profile/" + props.user.user_id} className="ml-16 hover:underline text-lg">← Back to user's profile</Link>
            <div className='register_form_with_title my-24 w-4/12 py-16 px-24 rounded-lg bg-gray-200 mx-auto'>
                <h1 className='text-center text-4xl font-bold mb-8'>Edit user's details</h1>
                ...
                <form onSubmit={submit} className='flex flex-col gap-y-4 mx-auto'>
                    ...
                    <div className='flex flex-col'>
                        <label htmlFor="name">Role<span className='text-red-500'>*</span></label>
                        <input className='p-1 border-1 border-black rounded-md' type="text" name="role" id="role" required value={data.role} onChange={e => setData('role', e.target.value)} />
